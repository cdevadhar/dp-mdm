...
def calc_sample_norms(
    ...
    if flat:
        ...
        # NEW CODE
        try:
            # Attempt to stack norms and compute a single flat norm
            norms = [torch.stack(norms, dim=0).norm(2, dim=0)]
        except RuntimeError as e:
            print(f"Cannot stack tensors due to varying shapes: {e}")

            # Pad tensors with 0 to be able to stack tensors
            max_size = max(tensor.shape[0] for tensor in norms)
            padded_norms = [F.pad(tensor, (0, max_size - tensor.shape[0])) for tensor in norms]

            # Try again
            norms = [torch.stack(padded_norms, dim=0).norm(2, dim=0)]
